...
/**
 * 调用的方法为js形如：window.location.href="jsb://queryDownload/123/callback?apkid=16771755";
 * 分别是：jsb://方法名/系列号/回调方法?参数key=value
 * 回调方法如不需要可以不用。但是需要调用的方法名和系列号必须要
 */
public class JsBridge {

    public static final String TAG = JsBridge.class.getName();
    // 默认值
    ...
    public void response(String function, int seqid, String method, String result, Map<String, String> extMap, ResponseType type){
        if(TextUtils.ckIsEmpty(function)){
            return;
        }
    ...
    public void responseFail(String callbackFun, int seqid, String method, int code, Map<String, String> extMap, ResponseType type){
        if(TextUtils.ckIsEmpty(callbackFun)){
            return;
        }
        JSONObject json = new JSONObject();
        try {
